...
export class RegCompanies extends HTMLElement {
  ...
  render() {
    ...
    <div class="card mt-3">
      <div class="card-header">
        Registrar Company <span class="badge rounded-pill text-bg-primary" id="idView"></span>
      </div>
      <div class="card-body">
        <form id="frmDataCompanies">
          <div class="row">
            <div class="col">
              <label class="form-label" for="idCountryC">País</label>
              <select class="form-select" id="idCountryC" name="idCountryC"></select>
            </div>
            <div class="col">
              <label class="form-label" for="idRegionC">Región</label>
              <select class="form-select" id="idRegionC" name="idRegionC"></select>
            </div>
            <div class="col">
              <label class="form-label" for="idCity">Ciudad</label>
              <select class="form-select" id="idCity" name="idCity"></select>
            </div>
          </div>
  ...
  ctrlBtn = (json) => {
    ...
    data[0].forEach(sel => { const b=this.querySelector(sel); if(b) b.classList.remove('disabled'); });
    data[1].forEach(sel => { const b=this.querySelector(sel); if(b) b.classList.add('disabled'); });
  }
  enabledBtns = () => { this.querySelectorAll(".btn").forEach(v => this.ctrlBtn(v.dataset.ed)); }

  // ===== CRUD =====
  saveData = () => {
    const frm = this.querySelector('#frmDataCompanies');
    this.querySelector('#btnGuardar').addEventListener('click', async (e)=>{
      e.preventDefault();
      const datos = Object.fromEntries(new FormData(frm).entries());
      const required = ['idCity','nombreCompany','nit','email','address'];
      for(const k of required){ if(!datos[k]){ alert('Todos los campos son obligatorios'); return; } }
      const r = await postCompanies(datos);
      if(!r.ok){ const t = await r.text().catch(()=> ''); alert(`No se pudo guardar (HTTP ${r.status}). ${t}`); return; }
      const data = await r.json();
      this.viewData(data.id);
      window.dispatchEvent(new CustomEvent('data-changed',{detail:{type:'companies'}}));
      this.disableFrm(true);
      this.ctrlBtn(this.querySelector('#btnGuardar').dataset.ed);
    });
  }
  eventoEditar = () => {
    this.querySelector('#btnEditar').addEventListener('click', async (e)=>{
      e.preventDefault();
      const id = this.querySelector('#idView').textContent;
      if(!id){ alert('No hay registro seleccionado'); return; }
      const frm = this.querySelector('#frmDataCompanies');
      const datos = Object.fromEntries(new FormData(frm).entries());
      const r = await patchCompanies(id, datos);
      if(!r.ok){ alert('No se pudo editar'); return; }
      window.dispatchEvent(new CustomEvent('data-changed',{detail:{type:'companies'}}));
    });
  }
  eventoEliminar = () => {
    this.querySelector('#btnEliminar').addEventListener('click', async (e)=>{
      e.preventDefault();
      const id = this.querySelector('#idView').textContent;
      if(!id){ alert('No hay registro seleccionado'); return; }
      const r = await deleteCompanies(id);
  ...
  // ===== estado form =====
  disableFrm = (estado) => {
    const frm = this.querySelector('#frmDataCompanies'); if(!frm) return;
  ...
  async loadCountries(){
    try{
      ...
      const selC = this.querySelector('#idCountryC');
      const selR = this.querySelector('#idRegionC');
      const selCi = this.querySelector('#idCity');
      selC.innerHTML=''; selR.innerHTML=''; selCi.innerHTML='';
      if(!data.length){ selC.disabled=true; selR.disabled=true; selCi.disabled=true; return; }
      data.forEach(c=>{ const o=document.createElement('option'); o.value=c.id; o.textContent=c.nombreCountry; selC.appendChild(o); });
      selC.disabled=false;
      await this.loadRegions(selC.value);
      await this.loadCities(selC.value, selR.value);
      selC.onchange = async (e)=>{ await this.loadRegions(e.target.value); await this.loadCities(e.target.value, selR.value); };
      selR.onchange = async (e)=>{ await this.loadCities(selC.value, e.target.value); };
  ...
  async loadRegions(countryId){
    try{
      ...
      const selR = this.querySelector('#idRegionC'); selR.innerHTML='';
      if(!regs.length){ selR.disabled=true; return; }
      regs.forEach(rg=>{ const o=document.createElement('option'); o.value=rg.id; o.textContent=rg.nombreRegion; selR.appendChild(o); });
      selR.disabled=false;
    }catch(e){ console.error(e); }
  }
  async loadCities(countryId, regionId){
    try{
      ...
      const selCi = this.querySelector('#idCity'); selCi.innerHTML='';
      if(!cs.length){ selCi.disabled=true; return; }
      cs.forEach(c=>{ const o=document.createElement('option'); o.value=c.id; o.textContent=c.nombreCity; selCi.appendChild(o); });
      selCi.disabled=false;
    }catch(e){ console.error(e); }
  }
}
