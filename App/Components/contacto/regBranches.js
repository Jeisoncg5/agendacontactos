...
export class RegBranches extends HTMLElement{
  ...
  render(){
    ...
    <div class="card mt-3">
      <div class="card-header">
        Registrar Branch <span class="badge rounded-pill text-bg-primary" id="idView"></span>
      </div>
      <div class="card-body">
  ...
  // CRUD
  saveData = () => {
    const frm = this.querySelector('#frmDataBranches');
    this.querySelector('#btnGuardar').addEventListener('click', async (e)=>{
      ...
      window.dispatchEvent(new CustomEvent('data-changed',{detail:{type:'branches'}}));
      this.disableFrm(true);
  ...
  eventoEditar = () => {
    this.querySelector('#btnEditar').addEventListener('click', async (e)=>{
      ...
      const frm=this.querySelector('#frmDataBranches');
      const datos=Object.fromEntries(new FormData(frm).entries());
      const r=await patchBranches(id, datos);
  ...
  eventoEliminar = () => {
    this.querySelector('#btnEliminar').addEventListener('click', async (e)=>{
      ...
      const id=this.querySelector('#idView').textContent;
      if(!id){ alert('No hay registro seleccionado'); return; }
      const r=await deleteBranches(id);
  ...
  disableFrm = (estado) => {
    const frm=this.querySelector('#frmDataBranches'); if(!frm) return;
  ...
  async loadCompanies(){
    try{
      ...
      if(!cs.length){ sel.disabled=true; this.querySelector('#btnGuardar').disabled=true; return; }
